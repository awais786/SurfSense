...
class TestProxyAuthMiddlewareDispatch:
    ...
    async def test_kill_switch_off_passes_through_without_db(self):
        """
        GIVEN  kill switch is off (enabled=False)
        WHEN   any request arrives
        THEN   call_next is called, proxy_user is never set, DB never touched
        """
    ...
    # ── SPEC 10: race condition / IntegrityError ──────────────────────────────
